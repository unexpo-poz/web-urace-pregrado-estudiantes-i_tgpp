...
<head>
	...
	$sql2="select CI_E, TIPO_PASANTIA from sol_tgpp where CI_E = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2";
        $conex1 -> ExecSQL($sql2,__LINE__,true);
	$fila0 = $conex1->filas;
	if ($fila0==1)
	{
	    //printf('El estudiante ya se encuentra Registrado en la Base de Datos');
		print (" <script language='javascript'> window.alert('El estudiante ya se encuentra Registrado en la Base de Datos');window.close();</script>");
	}
	    else
		{
		  ...
		  //if($_SESSION['especialidad']==5) {
			...
			# REGISTRA LA SOLICITUD DEL ESTUDIANTE

			$sql1="SELECT exp_e FROM dace002 WHERE ci_e = '$_SESSION[cedula]'";
			$conex -> ExecSQL($sql1,__LINE__,true);
			$exp_e = $conex->result[0][0];
			
			$conex1 -> iniciarTransaccion("Inicia Transaccion - ".$_SESSION[cedula]);
		    $sql2="INSERT INTO sol_tgpp (ci_e, tipo_pasantia, empresa, n_proyecto, fecha_i, fecha_c, fecha_l, cedula_ti, nombre_ti, apellido_ti, telefono_ti, correo_ti,id_estatus,c_uni_ca,exp_e,lapso) VALUES ('$_SESSION[cedula]', '$_REQUEST[TIPO_PASANTIA]', '$_SESSION[empresa]', '$_REQUEST[nom_proyecto]', '$_REQUEST[Sel_Fecha1]', '$_REQUEST[Sel_Fecha]', '$fecha_l','$_REQUEST[cedula_ti]', '$_REQUEST[nombre_ti]', '$_REQUEST[apellido_ti]', '$_REQUEST[telefono_ti]', '$_REQUEST[correo_ti]', 1, '$_SESSION[especialidad]','$exp_e','$lapsoProceso')";
			$conex1 -> ExecSQL($sql2,__LINE__,true);
...
<body onunload="recarga()";>
	...
	<table align="center" border="0" width="750" id="table1" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
		...
		<?php
		    $sql2="select empresa, n_proyecto, TIPO_PASANTIA, cedula_TI, nombre_TI, apellido_TI, telefono_TI, correo_TI, fecha_i, fecha_c, id_estatus, cedula_TA from sol_tgpp where CI_E = '$_SESSION[cedula]' and TIPO_PASANTIA =2";
		    $conex1 -> ExecSQL($sql2,__LINE__,true);
		    $fila2 = $conex1->filas;
		    if ($fila2==1)
		    {
		...
		<tr>
			...
			#OJO cambiar para emitir planilla
		      if($reg['C_UNI_CA']==5 && $tab['TIPO_PASANTIA']==2)
		      {
		      ?>
			<!-- <table align="center" border="1" cellpadding="3" cellspacing="1" width="550" style="border-collapse: collapse;">
				...
				<td style="width: 500px;" nowrap="nowrap" bgcolor="#FFFFFF" colspan="5">
				    <div class="mat"><b>DATOS DEL TUTOR ACADÉMICO</b></div>
				</td>
			    </tr>
			    <tr>
				<td>
