...
<head>
	...
	if (isset($_REQUEST['Sel_Fecha1'])){
		$conex1 -> iniciarTransaccion("Inicia Transaccion - ".$_SESSION[cedula]);
		
		if (!empty($_REQUEST[nom_proyecto])){
			$uSQL="UPDATE sol_tgpp SET n_proyecto='".$_REQUEST[nom_proyecto]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[Sel_Fecha1])){
			$uSQL="UPDATE sol_tgpp SET fecha_i='".$_REQUEST[Sel_Fecha1]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[Sel_Fecha])){
			$uSQL="UPDATE sol_tgpp SET fecha_c='".$_REQUEST[Sel_Fecha]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[cedula_ti])){
			$uSQL="UPDATE sol_tgpp SET cedula_ti='".$_REQUEST[cedula_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[nombre_ti])){
			$uSQL="UPDATE sol_tgpp SET nombre_ti='".$_REQUEST[nombre_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[apellido_ti])){
			$uSQL="UPDATE sol_tgpp SET apellido_ti='".$_REQUEST[apellido_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[telefono_ti])){
			$uSQL="UPDATE sol_tgpp SET telefono_ti='".$_REQUEST[telefono_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";$conex1 -> ExecSQL($uSQL,__LINE__,true);

		}

		if (!empty($_REQUEST[correo_ti])){
			$uSQL="UPDATE sol_tgpp SET correo_ti='".$_REQUEST[correo_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		$conex1 -> finalizarTransaccion("Finaliza Transaccion - ".$_SESSION[cedula]);
	}

	
	$sql2="select CI_E, TIPO_PASANTIA from sol_tgpp where exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1";
	...
	if ($fila==1)
	    ...
	    printf('');
			...
			//$sql2="INSERT INTO sol_tgpp (ci_e, tipo_pasantia, empresa, n_proyecto, fecha_i, fecha_c, fecha_l, cedula_ti, nombre_ti, apellido_ti, telefono_ti, correo_ti, id_estatus, c_uni_ca, exp_e, lapso) values ('$_SESSION[cedula]','$_REQUEST[TIPO_PASANTIA]','$_SESSION[empresa]','$_REQUEST[nom_proyecto]','$_REQUEST[Sel_Fecha1]','$_REQUEST[Sel_Fecha]','$fecha_l','$_REQUEST[cedula_ti]','$_REQUEST[nombre_ti]','$_REQUEST[apellido_ti]','$_REQUEST[telefono_ti]','$_REQUEST[correo_ti]',1,'$c_uni_ca','$exp_e','$lapsoProceso')";
	...
	<table align="center" border="0" width="750" id="table1" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
		...
		<tr>
			...
			<table border="0" width="650" cellpadding="0" align="center">
				...
				<td width="500">
				    ...
				    <p class="titulon">ASIGNATURA PRÁCTICA PROFESIONAL</p>
				</td>
				<td width="125">&nbsp;</td>
			    </tr>
		...
		<tr>
			...
			<table align="center" border="0" cellpadding="0" cellspacing="1" width="550">
			    <tr><td class="dp">&nbsp;</td></tr> 
			    <tr><td class="dp" style="text-align: right;"><?php date_default_timezone_set('Etc/GMT+4'); echo('Fecha : '.date('d').' de '.$meses[date('m')-1].' de '.date('Y').' Hora : '.date('h:i a')); ?></td></tr>
			    <tr><td class="dp">&nbsp;</td></tr> 
			</table>
	             </td>
	        </tr>
		    ...
		    <td width="750">
				...
				if ($fila==1)
					...
					'<tr>  
					    ...
					    <td style="width: 570px;" bgcolor="#FFFFFF"><div class="datosc">Teléfono: %s</div></td>
			...
			<table align="center" border="1" cellpadding="3" cellspacing="1" width="550" style="border-collapse: collapse;">
				...
				<td>
					...
					<tr>
					    ...
					    <td style="width: 100px;" nowrap="nowrap" bgcolor="#FFFFFF"><div class="matB">FECHA DE CULMINACIÓN</div></td>
			...
			<table align="center" border="1" cellpadding="3" cellspacing="1" width="550" style="border-collapse: collapse;">
				...
				<td>
				    <table border="0" cellpadding="0" cellspacing="1" width="550">
					    ...
					    <td style="width: 200px;" bgcolor="#FFFFFF"><div class="mat"><b>CÉDULA</b></div></td>
			                </tr>
					<tr>
					<?php
					...
					<tr>
					    <td style="width: 200px;" bgcolor="#FFFFFF"><div class="mat"><b>TELÉFONO</b></div></td>
