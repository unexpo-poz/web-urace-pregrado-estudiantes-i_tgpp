...
<body>
	...
	$conexion = mysql_connect("localhost","root","") or die ("Problemas en la conexión");
		...
		$registros = odbc_exec("select APELLIDOS, NOMBRES, EXP_E, C_UNI_CA from dace002 where CI_E = '$_SESSION[cedula]'" , $conexion)or die ("Problemas en el select");
    ?>
		...
		<tr align="center">
		    <td colspan="5">
				...
				<?php
				    $esp = odbc_exec("select CARRERA from tblaca010 where C_UNI_CA= '$reg[C_UNI_CA]'" , $conexion)or die ("Problemas en el select");
					...
					$datos = odbc_exec("select empresa, n_proyecto, cedula_ti, nombre_ti, apellido_ti, telefono_ti, correo_ti, fecha_i, fecha_c, fecha_l, estatus, cedula_TA, cedula_JC, cedula_JS from sol_tgpp where cedula_e = '$_SESSION[cedula]'" , $conexion)or die ("Problemas en el select");
		...
		if ($datostp['estatus']=="APROBADO" && $datostp['cedula_TA']!="")
		    {
		    $conexion = odbc_connect($DSN, $user, $pass, true, $laBitacora) or die ("Problemas en la conexión"); 	
		    $asc = odbc_exec("select APELLIDO, NOMBRE from tblaca007 where CI = '$datostp[cedula_TA]'" , $conexion)or die ("Problemas en el select");
		...
		if ($datostp['estatus']=="APROBADO" && $datostp['cedula_JC']!="" && $datostp['cedula_JS']!="")
		    {
		    $asc = odbc_exec("select APELLIDO, NOMBRE from tblaca007 where CI = '$datostp[cedula_JC]'" , $conexion)or die ("Problemas en el select");
		    if ($prof = odbc_fetch_array($asc)) 
			{
		?>
		<tr><td colspan="5"><font style="font-size:2px;">&nbsp;</font></td></tr>
		...
		<?php
		    ...
		    $asc = odbc_exec("select APELLIDO, NOMBRE from tblaca007 where CI = '$datostp[cedula_JS]'" , $conexion)or die ("Problemas en el select");
		    if ($prof = odbc_fetch_array($asc)) 
			{
		?>
		<tr><td colspan="5"><font style="font-size:2px;">&nbsp;</font></td></tr>
