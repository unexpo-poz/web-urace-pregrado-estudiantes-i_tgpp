...
<body onload="clickMenu('menu')">
	...
	$sql1="select CI_E, NOMBRES, APELLIDOS, C_UNI_CA, EXP_E from dace002 where CI_E = '$_SESSION[cedula]'";
	...
	<tr align="center">
		...
		<tr colspan="3">
			...
			<tr height="280" style="background: url(imagenes/fondo_unexpox.png) no-repeat">
				...
				<li class="sub">INICIO
				    <ul>
					<li><a href="http://localhost/webfinal/modest.php"></a></li>
				    </ul>
				</li>
				<li class="sub">MARCO LEGAL
					...
					<li class="fly"><a href="#nogo">Leyes y Reglamentos</a>
					    <ul>
					        <li><a href="pdf/Reglamento.pdf" target="popup" onclick="window.open('', 'popup', 'width = 790, height = 800, left=125, top=0')">Reglamento Vigente</a></li>
					    </ul>
					</li>	
				    </ul>
				</li>
				...
				<li class="sub">SOLICITUDES
				    <ul>
				        <li></a></li>
				        <li class="fly"><a>Registrar Anteproyecto</a>
					    <ul>
						<?php
						$flagtg=0;
						$flagpp=0;
						$sql2="select TIPO_PASANTIA from sol_tgpp where EXP_E = '$_SESSION[exp_e]'and TIPO_PASANTIA = 2";
						...
						$sql2="select TIPO_PASANTIA from sol_tgpp where EXP_E = '$_SESSION[exp_e]'and TIPO_PASANTIA = 1";
						$conex1 -> ExecSQL($sql2,__LINE__,true);
						$fila = $conex1->filas;
						if ($fila==1)
						{
					...
					<ul>
				        <li></a></li>
				        <li class="fly"><a>Modificar Anteproyecto</a>
					    <ul>
						<?php
						/*$flagtg=0;
						$flagpp=0;
						...
						{
						?>
						    <li><a href="planillatm.php" target="popup" onclick="window.open('', 'popup', 'width = 790, height = 800, left=125, top=0')">Trabajo de Grado</a></li>
						<?php
						}
						if($flagpp==1)
						{
						?>
						    <li><a href="planillapm.php" target="popup" onclick="window.open('', 'popup', 'width = 790, height = 800, left=125, top=0')">Práctica Profesional</a></li>
						<?php
						}
						?>
					    </ul>
				...
				<li class="sub">ESTADO DE TRÁMITES
				    <ul>
					    ...
					    <ul>
						<?php
						$sql2="select CI_E from sol_tgpp where EXP_E = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2";
						$conex1-> ExecSQL($sql2,__LINE__,true);
						$aux = $conex1->result;
						if($aux1 = $aux)
						{
						?>
						    <li><a href="datosesttg1.php" target="popup" onclick="window.open('', 'popup', 'width = 680, height = 800, left=195, top=0', scrollbars=yes)">Trabajo de Grado</a></li>
						<?php
						}
						$sql2="select CI_E from sol_tgpp where EXP_E = '$_SESSION[exp_e]' and TIPO_PASANTIA = 1";
						$conex1-> ExecSQL($sql2,__LINE__,true);
						$aux2 = $conex1->result;
						if($aux3 =$aux2)
						{
						?>
						    <li><a href="datosestpp1.php" target="popup" onclick="window.open('', 'popup', 'width = 680, height = 800, left=195, top=0', scrollbars=yes)">Práctica Profesional</a></li>
						<?php
						}
						?>
					    </ul>
