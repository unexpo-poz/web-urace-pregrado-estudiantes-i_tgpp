...
<head>
	...
	if (isset($_REQUEST['Sel_Fecha1'])){
		...
		if (!empty($_REQUEST[nom_proyecto])){
			$uSQL="UPDATE sol_tgpp SET n_proyecto='".$_REQUEST[nom_proyecto]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[Sel_Fecha1])){
			$uSQL="UPDATE sol_tgpp SET fecha_i='".$_REQUEST[Sel_Fecha1]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[Sel_Fecha])){
			$uSQL="UPDATE sol_tgpp SET fecha_c='".$_REQUEST[Sel_Fecha]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[cedula_ti])){
			$uSQL="UPDATE sol_tgpp SET cedula_ti='".$_REQUEST[cedula_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[nombre_ti])){
			$uSQL="UPDATE sol_tgpp SET nombre_ti='".$_REQUEST[nombre_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[apellido_ti])){
			$uSQL="UPDATE sol_tgpp SET apellido_ti='".$_REQUEST[apellido_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}

		if (!empty($_REQUEST[telefono_ti])){
			$uSQL="UPDATE sol_tgpp SET telefono_ti='".$_REQUEST[telefono_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);

		}

		if (!empty($_REQUEST[correo_ti])){
			$uSQL="UPDATE sol_tgpp SET correo_ti='".$_REQUEST[correo_ti]."' WHERE  exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2 ";
			$conex1 -> ExecSQL($uSQL,__LINE__,true);
		}
	...
	$sql2="select CI_E, TIPO_PASANTIA from sol_tgpp where exp_e = '$_SESSION[exp_e]' and TIPO_PASANTIA = 2";
...
<body>
	...
	<table align="center" border="0" width="750" id="table1" cellspacing="1" cellpadding="0" style="border-collapse: collapse">
		...
		<?php
		    	...
		    	$sql2="select empresa, n_proyecto, cedula_TI, nombre_TI, apellido_TI, telefono_TI, correo_TI, fecha_i, fecha_c, id_estatus from sol_tgpp where CI_E = '$_SESSION[cedula]' and TIPO_PASANTIA = 2";
			$conex1 -> ExecSQL($sql2,__LINE__,true);
